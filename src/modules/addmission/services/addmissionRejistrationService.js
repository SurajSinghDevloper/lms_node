...
const AdmissionService = {
  async newRegistration(dto) {
    try {
      ...
      const existingRegistration = await Admission.findOne({
        email: dto.email,
      });
      ...
      const emailResult = await sendCompleteRegistrationEmail(
        savedAdmission.email,
        userPassword
      );
  ...
  async updatePasswordOfUsers() {
    try {
      ...
      for (const user of allUsers) {
        ...
        if (register) {
          console.log(
            `Updating password for ${(user.email, " <.>", register.password)}`
          );
